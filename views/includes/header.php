...
$menuItems = include __DIR__ . '/../../config/menu.php'; 
?>
<!DOCTYPE html>
<html lang="es">
<head>
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center space-x-4">
                    <?php foreach ($menuItems as $item): ?>
                            ...
                            <div class="relative" x-data="{ open: false }" @click.away="open = false">
                                ...
                                <div x-show="open" x-cloak 
                                    ...
                                    <?php foreach ($item['submenu'] as $sub): ?>
                                        <a href="<?= $sub['url'] ?>" class="block px-4 py-2 text-sm hover:bg-indigo-50 hover:text-indigo-600">
                                            <?= $sub['label'] ?>
                                        </a>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        <?php else: ?>
                            <a href="<?= $item['url'] ?>" class="px-3 py-2 rounded-md text-sm font-medium hover:bg-indigo-500 transition">
                                <?= $item['label'] ?>
                            </a>
                        <?php endif; ?>
                    <?php endforeach; ?>
        ...
        <div x-show="mobileMenuOpen" x-cloak class="md:hidden bg-indigo-700 border-t border-indigo-500 transition-all">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <?php foreach ($menuItems as $item): ?>
                    <?php if (isset($item['submenu'])): ?>
                        <div x-data="{ open: false }">
                            ...
                            <div x-show="open" class="pl-4 bg-indigo-800/50 rounded-md mt-1">
                                <?php foreach ($item['submenu'] as $sub): ?>
                                    <a href="<?= $sub['url'] ?>" class="block px-3 py-2 text-sm text-indigo-100 hover:text-white">
                                        <?= $sub['label'] ?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                    ...
                    <?php else: ?>
                        <a href="<?= $item['url'] ?>" class="block px-3 py-2 rounded-md text-base font-medium hover:bg-indigo-800">
                            <?= $item['label'] ?>
                        </a>
                    <?php endif; ?>
                <?php endforeach; ?>
