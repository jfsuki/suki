...
// app/Core/Agents/ConversationGateway.php
    ...
    final class ConversationGateway
    {
        ...
        private function isPureGreeting(string $text): bool
        {
            ...
            return $result['intent'] === 'greeting' && $result['score'] >= 0.75;
        }
        ...
        private function classify(string $text): string
        {
            ...
            $result = $this->intentClassifier()->classify($text);
            // Normalize a few LLM intent names to legacy names expected by localReply()
            $map = [
                'affirmation' => 'confirm',
                'farewell'    => 'thanks',
                'create_request' => 'crud',
        ...
        private function isFarewell(string $text): bool
        {
            ...
            $result = $this->intentClassifier()->classify($text);
            return $result['intent'] === 'farewell' && $result['score'] >= 0.75;
        }
        ...
        /**
         * FAST PATH entry point — called by the pipeline trait ambiguity guard.
         *
         * Instantiates BuilderFastPathParser and calls parse() with a compact payload.
         * PHP is the only executor; LLM only returns a translated JSON.
         * This method NEVER throws — any error returns the deterministic fallback.
         *
         * @param string $text        Normalized user input
         * @param string $step        Current onboarding step key
         * @param array  $profile     User profile (for known_fields summary)
         * @param array  $state       Current session state
         * @param array  $allowed     Step-specific allowed catalog values
         * @return array{intent:string, mapped_fields:array, reply:string, confidence:float, needs_clarification:bool, via:string}
         */
        private function builderFastPath(
            string $text,
            string $step,
            array $profile,
            array $state,
            ...
            // Prepare compact known/missing summary — only relevant fields, no history.
            $known = array_filter([
                'business_type'   => $profile['business_type'] ?? null,
                'operation_model' => $profile['operation_model'] ?? null,
                'needs_scope'     => $profile['needs_scope'] ?? null,
                'documents_scope' => $profile['documents_scope'] ?? null,
            ], fn($v) => $v !== null && $v !== '');
            ...
            try {
                ...
                return $parser->parse($text, $step, $known, $missing, $allowed);
            } catch (\Throwable $e) {
                // Guaranteed safe fallback — never null, never exception bubbles up.
                return [
                    'intent'              => 'ask_user',
                    'mapped_fields'       => [],
                    'reply'               => 'Cuéntame en qué puedo ayudarte con tu negocio.',
                    'confidence'          => 0.0,
                    'needs_clarification' => true,
                    'via'                 => 'fast_path_exception_guard',
                ];
            }
        }

        /**
         * Thin alias for builderLlmAllowedValuesForStep() used by the pipeline trait.
         * The pipeline trait must not know internal naming of the onboarding trait.
         */
        private function builderAllowedValuesForStep(string $step, array $profile, array $state): array
        {
            return $this->builderLlmAllowedValuesForStep($step, $profile, $state);
        }
    ...
    private function parseEntityFromCrudText(string $text): string
        ...
        private function telemetry(string $classification, bool $local, array $parsed = []): array
        {
            ...
            if ($this->intentClassifier !== null) {
                $clsArgs = $this->intentClassifier->getLastClassificationTelemetry();
                if (!empty($clsArgs['llm_usage'])) {
                    $base['llm_telemetry'] = $clsArgs['llm_usage'];
                    $base['resolved_locally'] = false; // Mistral fallback was triggered
                }
            }
